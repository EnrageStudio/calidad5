...
function Madeini_theme_customize_register($wp_customize) {
	...
	//Sidebar Color
	$wp_customize->add_setting('madeini_option[color][sidebar_menu]', array(
		'default'			=> '#b07b8b',
		'sanitize_callback'	=> 'sanitize_hex_color',
		'capability'		=> 'edit_theme_options',
		'type'				=> 'option',
	));
	$wp_customize->add_control( new WP_Customize_Color_Control($wp_customize, 'madeini_option[color][sidebar_menu]', array(
		'label'				=> 'Sidebar menu color',
		'section'			=> 'colors',
		'settings'			=> 'madeini_option[color][sidebar_menu]',
		'priority'			=> 103,
	)));
	...
	//Sidebar bottom line
	$wp_customize->add_setting('madeini_option[color][sidebar_menu_active]', array(
		'default'			=> '#b07b8b',
		'sanitize_callback'	=> 'sanitize_hex_color',
		'capability'		=> 'edit_theme_options',
		'type'				=> 'option',
	));
	$wp_customize->add_control( new WP_Customize_Color_Control($wp_customize, 'madeini_option[color][sidebar_menu_active]', array(
		'label'				=> 'Sidebar Menu Active Color',
		'section'			=> 'colors',
		'settings'			=> 'madeini_option[color][sidebar_menu_active]',
		'priority'			=> 116,
	)));
...
function Madeini_customize_css(){
	...
	if( empty( $options['color']['navigation'] ) ){
		echo "#masthead .header-main { background-color:#b07b8b !important;  }";
	}else{
		echo "#masthead .header-main { background-color:".$options['color']['navigation']." !important;  }";
	}
	
	if( empty( $options['color']['sidebar_menu'] ) ){
		echo "#menu-barra-lateral li { background-color:#b07b8b; }";
	}else{
		echo "#menu-barra-lateral li { background-color:".$options['color']['sidebar_menu']."; }";
	}
	if( empty( $options['color']['sidebar_menu_active'] ) ){
		echo ".secondary-navigation .current-menu-item > a{background: #036eaa}";
	}else{
		echo ".secondary-navigation .current-menu-item > a{background: ".$options['color']['sidebar_menu_active']."}";
	}
...
function madeini_admin_menu() {
    add_theme_page( 'GMO WP Clowd', 'More', 'read','more', 'madeini_menu_more' );
}
...
wp_add_dashboard_widget('madeini_dashboard_widget', 'GMO WP Cloud', 'madeini_dashboard_widget_function');
